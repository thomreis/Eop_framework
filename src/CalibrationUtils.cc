...
void DrawCorr_EE(TH2F* h_scale_EEM, TH2F* h_scale_EEP,
                 ...
                 std::vector<std::vector<TGraphErrors*> >& correctionMomentum,
		 TEndcapRings* eRings, bool skip , int nEtaBinsEE, float etaMin, float etaMax)
{
...
///////////////////////////////////////////////////////////////////
TGraphErrors* NormalizeIC_EB(TH2F* h_scale_EB, TH2F* hcmap_EB,
			     const std::vector< std::pair<int, int> > & TT_centre,
			     bool skip, float ICmin, float ICmax)
{
                         ...
                         TH2F* hcmap, TH2F* hcmap2)
{
  ...
  for(int jbin = 1; jbin <= hcmap->GetNbinsY(); ++jbin) {
    ...
    //std::cout<<"jbin="<<jbin<<" --> etaring="<<etaRing<<std::endl;
    if( etaRing == -1 ) {
      //std::cout<<"etaring is -1"<<std::endl;
      continue;
    }
    ...
    for(int ibin = 1; ibin <= hcmap->GetNbinsX(); ++ibin) {
      ...
      if( hcmap2 == NULL ) {
	//std::cout<<"hcmap2 is NULL"<<std::endl;
	...
	if( IC1 > 0. && IC1 < 2. && IC2 > 0. && IC2 < 2. ) {
	  //std::cout<<"passed check size"<<IC2<<std::endl;
	  //std::cout<<"filling h_spread_vsEta at etaring="<<etaRing<<" with "<<(IC1 - IC2) / (IC1 + IC2)<<std::endl;
  ...
  for(int etaRing = 0; etaRing < nEtaRings; ++etaRing) {
    ...
    //std::cout<<"starting fit vs eta for etaring"<<etaRing<<std::endl;
    //std::cout<<"N events in histo = "<<h_spread_vsEta[etaRing] -> GetEntries()<<std::endl;    
    h_spread_vsEta[etaRing] -> Fit(funcName, "QLS+", "", center - 3.*h_spread_vsEta[etaRing]->GetRMS(), center + 3.*h_spread_vsEta[etaRing]->GetRMS());
    //std::cout<<"ending fit vs eta"<<std::endl;
